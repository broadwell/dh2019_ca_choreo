...
The following code blocks run the two analytical tasks defined above on excerpts of videos and generate visualizations of their results. Feel free to experiment with the settings, or with the code itself.

```{r solo_comparison}
# First do a choreography cross-comparison analysis on a solo video
...
```{r another_solo}
# This one is shorter, so we can use the longer-running
# but more accurate distance-based metric
...
# Finally, do a frame-by-frame pose similarity comparison
# on a multi-dancer video
# Note that Laplacian similarity values are inverted
